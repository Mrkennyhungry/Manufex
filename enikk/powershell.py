...
def minimized_startupinfo() -> dict[str, Any]:
    """让子进程（powershell 控制台窗口）**最小化且不抢焦点**地启动。

    runner 会反复拉起 setup_env / uninstall_ioa_force / switch_ioa_hosts 等脚本，
    这些控制台窗口若正常显示，会闪到最前面并盖住 iOA 客户端，干扰视觉指纹与
    截图（实测：被浏览器/终端遮挡时客户端区域 OCR 拿到空文本，整步失败）。
    返回可直接 `**` 展开进 subprocess 调用的 kwargs；非 Windows 返回空 dict。
    """
    if os.name != "nt":
        return {}
    try:
        ...
        si.wShowWindow = 7  # SW_SHOWMINNOACTIVE：最小化，且不激活（不抢焦点）
